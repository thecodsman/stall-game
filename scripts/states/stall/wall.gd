...
func physics_update(delta : float) -> void:
	player.on_wall_prev_frame = true
	check_for_wall_jump()
	...
	if player.is_on_wall_only() && sign(player.direction) != wall_dir && player.on_wall_prev_frame:
		player.on_wall_prev_frame = false
		await get_tree().create_timer(0.075).timeout
		if state_machine.state != self: return
		finished.emit("Air")
	elif !player.is_on_wall_only() && player.on_wall_prev_frame:
		player.on_wall_prev_frame = false
		await get_tree().create_timer(0.075).timeout
		if state_machine.state != self: return
		finished.emit("Air")
