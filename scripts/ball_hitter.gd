...
func kick(ball_path : NodePath, dir : Vector2) -> void:
	...
	if ball.combo >= 4:
		...
		tween.tween_property(UI.combo_counter, "scale", Vector2(1,1), 0.25).from(Vector2(1.5,1.5))
		tween.tween_property(UI.combo_counter, "rotation", randf_range(-PI/6,PI/6), 0.25).from(0)
