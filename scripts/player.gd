	...
	match state:
		State.IDLE:
			...
			if direction:
				...
				if abs(direction) > 0.5: set_state.rpc_id(1,State.RUN_DASH)
				else: set_state.rpc_id(1,State.WALK)
			if not is_on_floor():
		...
		State.WALK:
			if not move(delta, ACCEL, SPEED * 0.65): set_state.rpc_id(1,State.IDLE)
			check_for_jump()
			check_for_drop_through()
			check_for_kick()
			check_for_special()
		...
		State.RUN:
			direction = input.direction.x
			if abs(direction) < dead_zone: direction = 0
			if not direction && abs(velocity.x) < 1: set_state.rpc_id(1,State.IDLE)
		...
		State.AIR:
			...
			if is_on_floor() && velocity.y >= 0: set_state.rpc_id(1,State.IDLE)
			elif is_on_wall_only() && sign(velocity.x) == -get_slide_collision(0).get_normal().x: set_state.rpc_id(1,State.WALL)
		...
		State.STALL:
			...
			if anim_finished && not is_ball_stalled: set_state.rpc_id(1,State.AIR) # set the state to air to avoid coyote time
			...
			if kick_pressed: set_state.rpc_id(1,State.STALL_KICK)
...
func check_for_special():
	...
	if is_special_pressed:
		set_state.rpc_id(1,State.STALL)


# func check_for_jump():
# 	var is_jump_pressed = input.is_joy_button_pressed(controller_index, JOY_BUTTON_A)
# 	if is_jump_pressed && not j_prev_frame:
# 		if can_jump: set_state.rpc_id(1,State.JUMP)
# 		j_prev_frame = true
